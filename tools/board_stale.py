#!/usr/bin/env python3
"""git-date staleness walk over the board — the `make`-style blooming check.

A parked, bloomable task is STALE when a hard dependency's sidecar was committed more
...
def main():
    ...
    if '--json' in args:
        ...
        print(f"# {len(rows)} bloomable tasks {label}")
        for i, f, s in rows:

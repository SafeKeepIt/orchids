...
- [ ] Secrets retrieved via macOS Keychain on the operator's machine; Keychain keys
      documented; setup command shown to the operator when a key is missing
...
- **macOS: Keychain.** On macOS, scripts retrieve secrets from Keychain at runtime
  (e.g. `security find-generic-password`).
- **Other operating systems: operator handles manually.** On non-macOS hosts, secret
  operations are done by the operator manually; the script does not automate secret
  storage or retrieval there.
- **Document the keys.** The script's header lists the Keychain item names it expects.
